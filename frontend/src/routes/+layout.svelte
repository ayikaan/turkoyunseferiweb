<script lang="ts">
	...
	const themeNames: Record<string, Record<string, string>> = {
		...
		ru: { dark: 'Тёмная', light: 'Светлая', forest: 'Лесная', ocean: 'Океанская' },
		uk: { dark: 'Темна', light: 'Світла', forest: 'Лісова', ocean: 'Океанська' },
		zh: { dark: '深色', light: '浅色', forest: '森林', ocean: '海洋' },
...
{#snippet brandName()}
	<div class="brand-signature-logo">
		...
		{:else if currentLanguage.value === 'ru'}
			<span class="brand-bold">ИНИЦИАТИВА</span><span class="brand-light"> ТУРЕЦКИХ ИГР</span>
		{:else if currentLanguage.value === 'uk'}
			<span class="brand-bold">ІНІЦІАТИВА</span><span class="brand-light"> ТУРЕЦЬКИХ ІГОР</span>
		{:else}
			...
			<div class="nav-right-container" style="display: flex; align-items: center;">
				...
				<div class="lang-selector-container">
					<button class="lang-trigger-btn" class:active={themeDropdownOpen} onclick={toggleThemeDropdown}>
						<span style="font-size: 1.05rem; display: inline-flex; align-items: center; justify-content: center;">{themes.find(th => th.id === currentTheme)?.icon || '🌙'}</span>
