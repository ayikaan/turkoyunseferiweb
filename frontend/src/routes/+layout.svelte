<script lang="ts">
	...
	$effect(() => {
		if (isAdmin && currentTheme !== 'dark' && currentTheme !== 'light') {
			setTheme('dark');
		}
	});
...
<div style="display: flex; flex-direction: column; min-height: 100vh;">
	{#if isAdmin}
		<header id="main-header" class="admin-header">
			<nav class="container admin-nav" style="display: flex; align-items: center; justify-content: center; gap: 40px; width: 100%; max-width: 900px; margin: 0 auto; position: relative; padding: 0 20px;">
				<!-- Left: Back Button -->
				<a href="/" class="btn-back" style="display: flex; align-items: center; gap: 6px; text-decoration: none; color: var(--text-secondary); font-size: 0.9rem; font-weight: 600; padding: 8px 16px; border: 1px solid var(--border-color); border-radius: 8px; background: var(--bg-surface); transition: all 0.25s; white-space: nowrap;">
				...
				<div style="display: flex; align-items: center; gap: 8px; white-space: nowrap;">
					<TurkishFlag />
					<div class="brand-signature-logo" style="display: flex; gap: 6px; font-size: 1.1rem;">
						<span class="brand-bold" style="color: var(--text-primary); font-weight: 800; letter-spacing: 0.5px;">TÜRKÇE OYUN SEFERİ</span>
						<span class="brand-light" style="color: var(--accent-color); font-weight: 800; text-shadow: 0 0 10px rgba(223, 177, 91, 0.3);">ADMİN PANELİ</span>
					</div>
				</div>
				...
				<div style="display: flex; align-items: center; gap: 12px; white-space: nowrap;">
					...
					<div class="lang-selector-container">
						<button class="lang-trigger-btn" class:active={dropdownOpen} onclick={toggleDropdown}>
							<LanguageFlag code={currentLanguage.value === 'tr' ? 'tr' : 'en'} />
							<span class="active-lang-name">{currentLanguage.value === 'tr' ? 'Türkçe' : 'English'}</span>
						...
						<div class="lang-dropdown-menu" class:open={dropdownOpen}>
							<button class="lang-item-btn" class:selected={currentLanguage.value === 'tr'} onclick={() => selectLang('tr')}>
								<LanguageFlag code="tr" />
								<span class="lang-item-name">Türkçe</span>
							</button>
							<button class="lang-item-btn" class:selected={currentLanguage.value === 'en'} onclick={() => selectLang('en')}>
					...
					<div class="lang-selector-container">
						<button class="lang-trigger-btn" class:active={themeDropdownOpen} onclick={toggleThemeDropdown}>
							<span style="font-size: 1.05rem; display: inline-flex; align-items: center; justify-content: center;">{currentTheme === 'light' ? '☀️' : '🌙'}</span>
							<span class="active-lang-name">{currentTheme === 'light' ? 'Aydınlık' : 'Karanlık'}</span>
						...
						<div class="lang-dropdown-menu" class:open={themeDropdownOpen}>
							<button class="lang-item-btn" class:selected={currentTheme === 'dark'} onclick={() => setTheme('dark')}>
								<span style="font-size: 1.05rem; display: inline-flex; align-items: center; justify-content: center;">🌙</span>
								<span class="lang-item-name">Karanlık</span>
							</button>
							<button class="lang-item-btn" class:selected={currentTheme === 'light'} onclick={() => setTheme('light')}>
								<span style="font-size: 1.05rem; display: inline-flex; align-items: center; justify-content: center;">☀️</span>
								<span class="lang-item-name">Aydınlık</span>
							</button>
						</div>
					</div>
				</div>
	...
	<footer class="main-footer" style="padding-top: 60px; padding-bottom: 40px;">
		{#if isAdmin}
			<!-- Admin Page Footer -->
			<div class="container" style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 20px; text-align: center; padding-bottom: 20px;">
				<div style="display: flex; gap: 30px; flex-wrap: wrap; justify-content: center; font-size: 0.8rem; color: var(--text-secondary);">
					<span>Sistem Durumu: <span style="color: #34d399; font-weight: 600;">Aktif</span></span>
					<span>Versiyon: <span>v1.2.0</span></span>
					<span>Veritabanı: <span>Turso DB (SQLite)</span></span>
					<span>Sunucu: <span>Cloudflare Pages</span></span>
				</div>
				<p style="color: var(--text-secondary); font-size: 0.8rem; max-width: 600px; line-height: 1.6; margin: 0;">
					Bu yönetim paneli aracılığıyla haber ekleme, düzenleme ve sistem ayarları güncellemeleri anında gerçekleştirilmektedir. Haber ekleme işlemlerinde yapay zeka destekli çeviriler arka planda otomatik olarak yürütülür.
				</p>
			</div>
			...
			<div class="container" style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 15px;">
				...
				<div style="display: flex; justify-content: center; align-items: center; width: 100%; font-size: 0.75rem; color: var(--text-secondary); gap: 4px;">
